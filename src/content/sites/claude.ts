...
function findClaudeComposerActionButton(): HTMLElement | null {
  const input = queryFallback(INPUT_SELECTORS);
  const composerRoot = input?.closest('fieldset') ?? input?.closest('form') ?? input?.parentElement ?? null;
...
export const claudeAdapter: SiteAdapter = {
  ...
  findSendButton(): HTMLElement | null {
    return findClaudeComposerActionButton();
  },
